...
<p align="center">
  <strong>폐허 거리용 게임 도킹형 자동화·킬 카운터 도구</strong><br/>
...
**Pipela**는 사냥터 창 옆에 도킹되는 오버레이·제어창으로, 반복 플레이 동작(리로드·탄약·라이드·용병·플레임 등)을 템플릿 매칭과 입력 자동화로 돕고, OCR 기반 **킬 카운터**로 사냥 통계를 실시간 추적합니다.
...
**게임 없이 UI만 보기 (개발):** `python main.py` 소스 실행 시 기본으로 제어창·킬 패널·상단 스트립이 화면 중앙에 뜹니다. 끄려면 `set PIPELA_DEV_UI=0` 후 실행.
...
- Pipela는 **개인 편의 도구**이며, 대상 게임·퍼블리셔 등 제3자와 **무관**합니다.
