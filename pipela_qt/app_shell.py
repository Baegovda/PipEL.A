...
def settings_breadcrumb_chrome_qss() -> str:
    """설정 탭 경로(허브·서브) — `QPushButton#pipelaBreadcrumbSeg` / `QLabel#pipelaBreadcrumbSep`."""
    return (
...
def frameless_outer_window_qss(*, object_name: str) -> str:
    ...
    return (
        ...
        f"  border-radius: {scale_px(2)}px;\n"
        f"}}\n"
    )


def kill_counter_floater_window_qss() -> str:
    """`kill_counter_window.PipelaQtKillCounterWindow` — 탭/터미널 없이 외곽선·스크롤만 (`control_frameless_window_qss` 부분집합)."""
    return (
...
def settings_hub_entry_button_qss() -> str:
    """설정 탭 허브 그리드 `QPushButton` — `HubCard` 라벨과 동일 타이포 + 카드 면.

    (액센트 세로막대 없음; `QFrame#hubCard` 와 면·테두리·호버를 맞춘다.)"""
    _hub_fpt = T.spt(12)
    _hub_rs = T.RADIUS_SM
    _ls = letter_spacing_qss()
    _pad = qss_pad_vh(10, 12)
    return (
        f"QPushButton {{ background-color: {T.CARD_BG}; color: {T.FG}; font-weight: 600; "
        f"font-size: {_hub_fpt}; font-family: {T.FONT_CSS_UI}; border: 1px solid {T.BORDER}; "
        f"padding: {_pad}; letter-spacing: {_ls}; border-radius: {_hub_rs}; text-align: center; }}"
        f"QPushButton:hover {{ background-color: {T.CARD_HOVER}; border: 1px solid {T.ACCENT}; color: {T.FG}; }}"
        f"QPushButton:pressed {{ background-color: {T.BTN_PRESSED}; border-color: {T.BORDER_HAIR}; }}"
    )
...
def control_frameless_window_qss() -> str:
    ...
    _sb_narrow = max(4, scale_px(5))
    _sb_cap_r = max(2, scale_px(3))
    _sb_edge = scale_px(3)
    _sb_handle_pad = scale_px(2)
    _sb_min = scale_px(22)
    _pane_pad = scale_px(4)
